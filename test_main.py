...
def test_view_events_with_events(db_connection):
    ...
    """Тест для функции view_events с доступными спектаклями."""
    cursor = db_connection.cursor()
    cursor.execute("INSERT INTO events (name, price, date, free_slots) VALUES (?, ?, ?, ?)",
                   ("Test Event", 100.0, "2023-10-01", 5))
    db_connection.commit()

    with patch('builtins.input', side_effect=["1"]), patch('builtins.print') as mock_print:
        view_events()
        mock_print.assert_any_call("Доступные спектакли:")
        mock_print.assert_any_call("ID: 1, Название: Test Event, Цена: 100.0, Дата: 2023-10-01, Свободные места: 5")
...
def test_cancel_ticket_with_order(db_connection):
    """Тест для функции cancel_ticket с существующим заказом."""
    cursor = db_connection.cursor()
    cursor.execute("INSERT INTO events (name, price, date, free_slots) VALUES (?, ?, ?, ?)",
                   ("Test Event", 100.0, "2023-10-01", 5))
    cursor.execute("INSERT INTO customers (name, age, email, phone) VALUES (?, ?, ?, ?)",
                   ("John Doe", 30, "john.doe@example.com", "1234567890"))
    
    cursor.execute('''
        INSERT INTO orders (customer_id, event_id, event_name, event_price)
        VALUES (?, ?, ?, ?)
    ''', (1, 1, "Test Event", 100.0))
    db_connection.commit()

    with patch('builtins.input', side_effect=["1"]), patch('builtins.print') as mock_print:
        cancel_ticket()
        cursor.execute("SELECT COUNT(*) FROM orders WHERE order_id = 1")
        count = cursor.fetchone()[0]
        assert count == 0  # Заказ должен быть удален
